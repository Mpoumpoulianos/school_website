...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($section_name)) {
        echo json_encode(['success' => false, 'error' => 'Άγνωστη ενότητα']);
        exit();
    }
    ...
    try {
        ...
        if($existing) {
            ...
            $stmt = $pdo->prepare("UPDATE page_content SET content = ?, last_modified = CURRENT_TIMESTAMP, modified_by = ? WHERE section_name = ? AND user_type = ?");
